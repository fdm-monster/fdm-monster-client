...
export const usePrinterFilesStore = defineStore({
  ...
  getters: {
    printerFileBucket(): (printerId: string) => PrinterFileBucket | undefined {
      return (printerId?: string) =>
        this.printerFileBuckets.find((p) => p.printerId === printerId);
    },
    printerFiles() {
  ...
  actions: {
    ...
    _clearPrinterFiles({
      ...
      const bucket = this.printerFileBuckets.find(
        (b) => b.printerId === printerId
      );
    ...
    _setPrinterFiles({
      ...
      let fileBucket = this.printerFileBuckets.find(
    ...
    _popPrinterFile({
      ...
      const fileBucket = this.printerFileBuckets.find(
        (p) => p.printerId === printerId
      );
      ...
      const deletedFileIndex = fileBucket.files.findIndex(
        (f) => f.path === fullPath
      );
